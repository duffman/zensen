...
import { MailboxDeleteResponse } from "imapflow";
import { AppSettings }           from "../Application.ts";
import { IIMAPConf }             from "../Application.ts";
import { CfgMailbox }            from "../Application.ts";
...
export class ImapManager {
	...
	constructor() {
		const cfgImap: IIMAPConf      = AppSettings.mail.imap;
		this.context                  = { config: CfgMailbox, logger: new Logger(true)};
		this.config                   = CfgMailbox.imap;

		const config: ImapFlowOptions = {
			host  : cfgImap.host,
			port  : cfgImap.port,
			secure: cfgImap.secure,
			auth  : {
				user: cfgImap.auth.user,
				pass: cfgImap.auth.pass
			},
		};
